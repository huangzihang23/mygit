...
class LLMReasoner:
    ...
    def GRPO_trl_wrapper(self, dataset, epoch):
        ...
        trainer = GRPOTrainer(
            model=self.model,
            reward_func=unified_reward_function,  # Use the new unified function
            args=training_args,
    ...
    def sft_finetune(self, dataset, epoch):
        """
        重写SFT微调函数，适配最新版trl库的SFTTrainer参数要求
        仅使用MCTS中成功且高质量（低PPL）的路径进行训练
        """
        lora_param = self.config['lora_conf']
        train_param = self.config['train_conf']
        ...
        sft_config = SFTConfig(
            ...
            fp16=train_param.get('fp16', True),
            report_to="tensorboard",
            max_steps=-1,  # 由num_train_epochs控制训练步数
            dataloader_drop_last=True,  # 确保批次大小一致
            ddp_find_unused_parameters=False,  # 分布式训练优化
            max_length=train_param.get('max_seq_length', 1024),  # 使用max_length而不是max_seq_length
            packing=False,  # 不使用packing，保持原始序列结构
            dataset_text_field="text",  # 指定数据集中的文本字段名称
        )
...
class MCTSEnhancedReasoner:
    ...
    def _create_chain_of_thought_sample(self, path, initial_env, is_success):
        prompt_text = f"Using the numbers {initial_env.initial_numbers}, create an equation that equals {initial_env.target}."
        ...
        prompt_template = AutoTokenizer.from_pretrained(self.config['model_path'])
        prompt = prompt_template.apply_chat_template(r1_prefix, tokenize=False, add_generation_prompt=True)

        thought_steps = []
        equation_parts = {i: str(n) for i, n in enumerate(initial_env.initial_numbers)}

        current_numbers = initial_env.initial_numbers.copy()
        for i in range(len(path) - 1):
            node = path[i][0]
            op_data = path[i+1][0].op
            if op_data is None: continue
            
            op_symbol, idx1, idx2, result = op_data
            
            if node.env.current_numbers[idx1] not in current_numbers or node.env.current_numbers[idx2] not in current_numbers:
                continue
            ...
            current_numbers = path[i+1][0].env.current_numbers.copy()
        ...
        if is_success:
            conclusion = f"The final equation results in {final_result}, which matches the target {initial_env.target}. The solution is correct."
        else:
            conclusion = f"The final equation results in {final_result}, which does not match the target {initial_env.target}. This is an incorrect path."
        ...
        return {
            "prompt": prompt, 
            "response": response, 
            "target": initial_env.target, 
            "initial_numbers": initial_env.initial_numbers
        }
...
if __name__ == "__main__":
    ...
    config['work_dir'] = config['work_dir'].format(model_path=config['model_path'].replace("/", "_"), ts=time_stamp)
