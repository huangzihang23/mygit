...
class LLMReasoner:
    ...
    def generate_prompt(self, initial_numbers, target):
        r1_prefix = [{
            ...
            "role": "user",
            "content": f"Using the numbers {initial_numbers}, create an equation that equals {target}. Show your thought steps in <think> </think> tags. And return the final equation and conclusion in <answer> </answer> tags. For example, <think>\nStep 1: Starting with numbers [45, 14, 39], I choose to calculate 39 - 14, which gives 25. The remaining numbers are now [45, 25].\nStep 2: Starting with numbers [45, 25], I choose to calculate 45 - 25, which gives 20. The remaining numbers are now [20].\n</think>\n<answer>\nThe final equation is (45 - (39 - 14)) = 20, which matches the target 20. The solution is correct.\n</answer>"
        }, {
            "role": "assistant",
    ...
    def sft_finetune(self, dataset, epoch):
        lora_param = self.config['lora_conf']
        train_param = self.config['train_conf']

        lora_config = LoraConfig(
            r=lora_param['lora_rank'],
            lora_alpha=lora_param['lora_alpha'],
            target_modules=lora_param['target_modules'],
            lora_dropout=lora_param['lora_dropout'],
            bias=lora_param['lora_bias'],
            task_type=lora_param['task_type']
        )

        # 2. 处理数据集：合并prompt和response为模型输入文本
        hf_dataset = HFDataset.from_list(dataset)
        
        # 合并prompt和response为'text'字段（SFTTrainer需要的格式）
        def merge_prompt_response(example):
            return {"text": example["prompt"] + example["response"]}
        
        hf_dataset = hf_dataset.map(
            merge_prompt_response,
            remove_columns=["prompt", "response"]  # 移除不需要的字段
        )

        # 3. 配置训练参数
        work_dir = self.config['work_dir']

        # 4. 初始化SFT Trainer（修正参数错误）


        # 创建SFTConfig来设置序列长度等参数，包含TrainingArguments的所有配置
        sft_config = SFTConfig(
            output_dir=f'{work_dir}sft_epoch={epoch}',
            ...
            num_train_epochs=train_param.get('sft_num_epochs', 3),  # SFT专用训练轮次
            logging_steps=train_param['logging_steps'],
            optim=train_param["optim"],
            save_strategy="epoch",
            remove_unused_columns=True,
            max_steps=-1,  # 由num_train_epochs控制训练步数
            dataloader_drop_last=True,  # 确保批次大小一致
            max_length=train_param.get('max_length', 512),  # 使用max_length而不是max_seq_length
        )

        trainer = SFTTrainer(
            model=self.model,
            args=sft_config,  # 使用SFTConfig而不是TrainingArguments
            train_dataset=hf_dataset,
            processing_class=self.tokenizer,  # 使用processing_class而不是tokenizer
            peft_config=lora_config,
            formatting_func=None,  # 不需要额外的格式化函数，数据已经是文本格式
        )

        # 5. 开始训练
        logger.info(f"Starting SFT training for epoch {epoch} with {len(hf_dataset)} samples")
        train_result = trainer.train()
        
        # 6. 记录训练指标
        metrics = train_result.metrics
        metrics["train_samples"] = len(hf_dataset)
        trainer.log_metrics("sft_train", metrics)
        trainer.save_metrics("sft_train", metrics)
        
        # 7. 保存合并后的模型
        save_path = os.path.join(work_dir, f"sft_epoch_{epoch}_final_model")
        ...
        self.model = AutoModelForCausalLM.from_pretrained(
            save_path,
            device_map="auto" if hasattr(self.model, 'device_map') else None
        )
            ...
            for i, completion in enumerate(completions):
                ...
                if think_match:
                    ...
                    for step_initial, step_calc, step_result_str, next_numbers_str in steps:
                        relative_position = step_idx / (len(steps) - 1) if len(steps) > 1 else 1.0
                        step_reward = 0.0
                        is_step_valid = False
                        try:
                            ...
                            if operands_available:
                                ...
                                if abs(calculated_value - step_result) < 1e-6:
                                    ...
                                    if next_numbers and sorted(next_numbers) == expected_next_numbers:
                                        is_step_valid = True
                                        step_reward += 0.5 / len(steps)
                                        current_numbers = next_numbers
                                    step_reward += 0.5 / len(steps) if abs(calculated_value - step_result) < 1e-6 else 0.0
                            r_it = pos_m * (relative_position + pos_n)
                            position_bonus = pos_gamma * torch.sigmoid(r_it)
                            sign = 1 if step_reward > 0 else -1
                            step_reward += sign * position_bonus.item()
                            logger.info(f"Completion {i}: Positional shaping applied, sign={sign}, reward: {step_reward-position_bonus:.2f} -> {step_reward:.2f}")
                        ...
                        if is_step_valid:
                            num_valid_steps += 1
                        step_idx += 1
                    if not steps and len(think_text.strip()) > 20:
                        process_reward = 0.1  # 鼓励非空推理
                logger.info(f"Completion {i}: Process reward: {process_reward:.2f} (valid_steps={num_valid_steps}, total_steps={len(steps)})")

                # 4. Length Bonus (Encouraging longer reasoning)
                length_bonus = min(1.0, len(steps) / (len(initial_numbers_list) - 1) if steps else 0.0)
...
class MCTSEnhancedReasoner:
    ...
    def _create_chain_of_thought_sample(self, path, initial_env, is_success):
        ...
        for i in range(len(path) - 1):
            ...
            operand1_str = equation_parts[idx1]
            operand2_str = equation_parts[idx2]

            sub_expression = f"({operand1_str} {op_symbol} {operand2_str})"
            
            next_equation_parts = []
            for index, part in enumerate(equation_parts):
                if index not in (idx1, idx2):
                    next_equation_parts.append(part)
            next_equation_parts.append(sub_expression)
            equation_parts = next_equation_parts
        ...
        response = f"<think>\n{final_thought}\n</think>\n<answer>\n{conclusion}\n</answer>"
        return {"prompt": prompt, "response": response, 'initial_numbers': initial_env.initial_numbers, 'target': initial_env.target}
    ...
    for epoch in range(config['mcts_conf']['num_rollout']):
        ...
        for step, example in enumerate(epoch_train_set):
            ...
            for path, is_success in all_paths:
                ...
                cot_samples_for_grpo.append({**cot_sample, "is_success": is_success})
